...
def print_gff(features, args):
    ...
    for ft in features:
        ...
        print("%s\t.\tCDS\t%s\t%s\t.\t%s\t.\t%s" %
              (ft['QueryId'], qstart, qend, strand, featureCol))
...
def print_gbk(features):
    ...
    for ft in features:
        ...
        print("%s\t.\tCDS\t%s\t%s\t.\t%s\t.\tID=%s" %
              (ft['QueryId'], qstart, qend, strand, ft['SubjectId']))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Create annotation file from kaamer tsv results')
    ...
    parser.add_argument(
        ...
        type=float,
        help=
        'minimum identity or kmatchidentity to call the annotation on protein',
        default=90.0)
    parser.add_argument(
        '--minLen',
        type=float,
        help=
        'minimum alignment length of CDS to call the annotation on protein',
        default=60)

    parser.add_argument('--kaamer_res',
                        type=str,
                        help='kaamer results TSV file')

    parser.add_argument(
        '--ftProduct',
        type=str,
        help='the column name in result file to report CDS product annotation',
        default="ProteinName")
    parser.add_argument(
        '--ftGene',
        type=str,
        help='the column name in result file to report CDS gene annotation',
        default="GeneName")

    parser.add_argument('--stream',
                        action='store_true',
                        help='stream kaamer results output (read from pipe)')
